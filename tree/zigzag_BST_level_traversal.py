...
def inorder_traversal(level_idx, root, cb):
    if root is None:
        return
    if root.left is not None:
        inorder_traversal(level_idx + 1, root.left, cb)
    cb(level_idx, root)
    if root.right is not None:
        inorder_traversal(level_idx + 1, root.right, cb)
...
def zigzag_level_order(root: Optional[TreeNode]) -> List[List[int]]:
    ...
    inorder_traversal(0, root, traverse_cb)
